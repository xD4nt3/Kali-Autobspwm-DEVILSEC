...
forge::apt_install \
    xorg xinit xclip xdotool xinput xterm xsel scrot maim feh nitrogen fastfetch \
...
# Post-install verification for critical packages
forge::step "Verifying critical packages (feh, fastfetch)"
for pkg in feh fastfetch xdotool; do
    if ! command -v "$pkg" >/dev/null 2>&1; then
        forge::warn "$pkg not found after apt-install — retrying alone..."
        sudo DEBIAN_FRONTEND=noninteractive apt-get install -y "$pkg" 2>/dev/null \
            && forge::ok "  ▸ $pkg" \
            || forge::warn "  ▸ $pkg still missing — run 'devilsec-repair' later"
    fi
done
